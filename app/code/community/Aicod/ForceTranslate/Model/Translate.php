...
class Aicod_ForceTranslate_Model_Translate extends Mage_Core_Model_Translate
{
    protected function _loadThemeTranslation($forceReload = false)
    {
        ...
        $data = $this->_getFileData($file);
        foreach($data as $v=>$vt) {
          if($this->_data[$scope.self::SCOPE_SEPARATOR.$v])
            continue;
          $this->_data[$scope.self::SCOPE_SEPARATOR.$v] = $vt;
        }

        $file = Mage::getDesign()->getLocaleFileName('translate_fallback.csv');
        $scope = 'AICOD_FALLBACK';
        $data = $this->_getFileData($file);
        foreach($data as $v=>$vt) {
          if($this->_data[$scope.self::SCOPE_SEPARATOR.$v])
            continue;
    ...
    protected function _getTranslatedString($text, $code)
    {
        ...
        $ret = parent::_getTranslatedString($text,$code);
